...
export class CompanyService extends ICompanyServices {
  async getCompaniesByJobAndSector(
    city: string,
    job: string,
    limit: number,
  ): Promise<Company[]> {
    const authInfo = await this.getAuthCredentials();
    const token = `${authInfo.token_type} ${authInfo.access_token}`;

    const { data } = await firstValueFrom(
      this.httpService
        .get(
          `https://api.francetravail.io/partenaire/labonneboite/v2/recherche?city=${city}&job=${job}&page_size=100`,
          {
            headers: {
    ...
    const filteredByHighPotential = companies.filter(
      (c) => c.is_high_potential,
    );
    const orderedByPotentialValue = filteredByHighPotential.sort(
      ({ hiring_potential: a }, { hiring_potential: b }) => b - a,
    );
    const limitResultsShown = orderedByPotentialValue.slice(
      0,
      limit ?? orderedByPotentialValue.length,
    );
